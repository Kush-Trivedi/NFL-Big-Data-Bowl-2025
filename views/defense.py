...
st.title(":red[Defensive] Strategies & Tendency")
...
def generate_defensive_strength_weakness():
    strength = random.sample([
        "Effective at disrupting offensive rhythm",
        "Creates turnover opportunities",
        "Limits opponent's scoring options",
        "Adaptable to different offensive styles",
        "Encourages team communication",
        "Protects the paint well",
        "Allows for quick transition to offense",
        "Masks individual defensive weaknesses"
    ], 5)

    weakness = random.sample([
        "Can be vulnerable to outside shooting",
        "Requires high stamina and conditioning",
        "May lead to foul trouble",
        "Depends on specific player skills",
        "Can be exploited by smart offensive schemes",
        "Difficult to maintain weaknessistency",
        "May struggle against versatile offenses",
        "Requires extensive practice and coordination"
    ], 5)

    return strength, weakness

# Function to generate and display cards
def display_defensive_cards(logo_folder, selected):
    counter = 0
    full_content = ""
    ...
    for logo_file in logo_folder.glob("*.png"):
        ...
        strength, weakness = generate_defensive_strength_weakness()
        ...
        with col1:
            # Download button for individual reports
            st.markdown(
    ...
    st.download_button(
        ...
        data=full_pdf,
        file_name="season_defensive.pdf",
        mime="application/pdf",
...
if selected == "Season":
    display_defensive_cards(logo_folder, selected)
    ...
    display_defensive_cards(logo_folder, f"Week {selected_week}")
